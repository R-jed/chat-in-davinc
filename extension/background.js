...
const PORTS = [8775, 8776, 8777, 8778, 8779];
...
function persist() {
  // Read the live connection state only when this queued write actually executes. A slow
  // discovery write that started before /pair completed must not publish its stale token=null
  // snapshot after the newly minted bearer has already been stored.
  const write = connectionWriteQueue.then(() => chrome.storage.local.set({ port, token, disconnected }));
  connectionWriteQueue = write.then(() => undefined, () => undefined);
  return write;
}
...
async function hello(candidate) {
  try {
    ...
    return body && body.app === 'chat-in-davinci' ? body : null;
  } catch {
    return null;
  }
}
...
/**
 * Reads presentation-only app state with the credential already in memory.
 *
 * This deliberately does not discover, provision, rotate a token, persist connection state,
 * or attach the wake socket. Opening the popup to learn its language must not become a browser
 * lifecycle action. The normal status/activity paths remain the only owners of that lifecycle.
 */
async function peekCurrent(path) {
  await load();
  if (disconnected || !token || port === null || portCompatible === false) return { ok: false };
  try {
    const response = await fetchBounded(
      `http://127.0.0.1:${port}${path}`,
      {
        method: 'GET',
        cache: 'no-store',
        headers: { ...versionHeaders(), authorization: `Bearer ${token}` }
      },
      REQUEST_TIMEOUT_MS
    );
    const data = await response.json().catch(() => ({}));
 ...
 *
 * First install is silent because the manifest key pins this unpacked companion to one
 * Stable Chrome Origin. That public identity is not a credential: after the first pair,
 * every rotation/reconnect also presents the current bearer, so copying the public key into
 * another unpacked extension cannot steal an established local pairing.
 */
...
function offerDesktopInput(tabId, message) {
  // The page reply is not a delivery receipt. Content owns one synchronous busy slot, then the
  // app's durable browser claim and one-time send authorization fence every later offer. Waiting
  // here can only starve unrelated repair work.
...
async function deliverDesktopInputs(inputs, background, reusableConversations = [], activeIds) {
  ...
  for (const input of inputs.slice(0, 50)) {
    ...
    if (!tab) {
      // Native New Chat can replace the document before the preparation reply reaches this
      // worker. The durable preparing election has already spent this input's one-tab authority,
      // so only the exact elected tab may resume after its replacement recorder proves a safe,
      // stable page. Never convert an ambiguous reply into authority for a fallback tab.
      if (!target && elected?.stage === 'preparing' && Number.isInteger(elected.tab)) {
        ...
        // tabConversations may still name the chat that existed before Native New Chat. A
        // concrete current/pending /c route can protect another conversation; stale registry
        // metadata on the exact elected home document cannot.
        ...
        if (candidate && !candidate.pendingUrl && modelCatalogTarget?.tab !== candidate.id &&
            (!concreteConversation || reusable.has(concreteConversation)) && ownsDocument(source)) {
          ...
          if (proof?.navigationEpoch === navigationEpoch && ownsDocument(source) && current &&
              !current.pendingUrl && current.url === candidate.url) {
...
async function maintainOnce() {
  ...
  // Reloading and offering input to the same document in one maintenance pass races two browser
  // mutations. Repair first; the app's durable input remains queued for the next status pass.
  ...
  for (const { conversationId, token, reason, focus, turnId } of repairs) {
    ...
    if (reason === 'stale-render') {
      // This handout belongs to the exact document and local turn that saw the frozen renderer.
      // Never reopen a missing tab, and let that same document synchronously recheck draft/tool
      // state before initiating its own reload.
      ...
      const source = { tab: target.id, documentId: tabDocuments[key], navigationEpoch: tabEpochs[key] };
 ...
 *
 * All callers here are automatic cleanup paths, never a direct user-close action. Re-read the
 * current tab immediately before removal because the user can pin it while earlier safety checks
 * are awaiting. Refusing the physical close does not undo retirement/supersession state; it only
 * preserves the tab the user explicitly chose to keep.
 */
 ...
 *
 * Named in one list rather than hand-copied ternaries because this body is rebuilt field by
 * field. A field nobody remembers to list is otherwise dropped silently with both ends of the
 * feature looking correct. `destinationLost` did exactly that.
 *
 * This stays an allowlist, not a passthrough: nothing reaches the app unless it is named here,
 * and every checkpoint stays token-paired to the continuation it describes.
 */
...
// marked response has grown. A bare count can never move a deadline by itself.
...
const HANDLERS = {
  ...
  async status() {
    ...
    return {
      ...
      paired: token !== null && !disconnected,
      disconnected,
  ...
  async unpair() {
    ...
    connectionEpoch++;
    // Keep the bearer while disconnected. It is unusable for ordinary requests, but it is the
    // continuity proof an explicit Connect sends if the app also froze that credential.
    disconnected = true;
    pairingError = null;
    await persist();
    return { ok: true };
  },
  /** Ask every eligible ChatGPT tab to rebuild its Chat in DaVinci activity stream now. */
  async overwriteNow() {
    await load();
  ...
  async compact(message, _sender, source) {
    ...
    // A checkpoint naming an existing chat must still prove the exact settled route. The
    // destination checkpoints deliberately name no conversation yet: they come from the
    // replacement document before ChatGPT has created /c/<id>, so loading/pendingUrl is not
    // evidence that this leased document is stale. In that case refuse only if it is already
    // showing some other conversation.
    if (named
  ...
  async goal_retry_helper(message, _sender, source) {
    await load();
    if (!ownsDocument(source)) return { ok: false, error: 'stale_document' };
    const conversationId = cleanConversationId(message.conversationId);
    const inputId = String(message.inputId || '');
    if (!conversationId || !/^[a-f0-9-]{36}$/i.test(inputId)) return { ok: false, status: 400, error: 'bad_goal_helper_retry' };
    await noteTabConversation(source, conversationId);
    if (!ownsDocument(source)) return { ok: false, error: 'stale_document' };
    const result = await call('/goal/helper/retry', {
      method: 'POST',
      body: JSON.stringify({ conversationId, inputId, clientId: String(source.tab) })
    });
    return ownsDocument(source) ? result : { ok: false, error: 'stale_document' };
  },
  /**
