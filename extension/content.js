...
(() => {
  ...
  let localLanguagePreference = 'system';
  let uiLanguage = I18N ? I18N.resolveLanguage(localLanguagePreference) : 'en';
  ...
  function localeForFormatting() {
    return uiLanguage === 'zh-CN' ? 'zh-CN' : 'en';
  }
  ...
  function refreshUiLanguage() {
    const next = I18N ? I18N.resolveLanguage(localLanguagePreference) : 'en';
    const changed = next !== uiLanguage;
    uiLanguage = next;
    return changed;
  }

  /** Repaint only extension-owned presentation after the effective UI language changes. */
  ...
  /** Localizes only presentation. Raw producer errors stay unchanged for control flow and diagnostics. */
  function presentationError(value, fallback) {
    const text = typeof value === 'string' ? value.trim().slice(0, 160) : '';
    if (!text) return '';
    if (uiLanguage !== 'zh-CN') return text;
    if (/[\u3400-\u9fff]/.test(text)) return text;
    if (/^[A-Za-z][A-Za-z0-9_.:-]{1,80}$/.test(text)) return fallback ? `${fallback} · ${text}` : text;
    return fallback || t('content.failed');
  }

  /** Translate only app-owned Goal failure shapes; provider/external detail stays raw. */
  function goalFailureText(value) {
    const text = typeof value === 'string' ? value.trim() : '';
    const exact = {
      'the app did not answer': 'content.appDidNotAnswer',
      'the model wrote nothing to open with': 'content.modelWroteNothing',
      'the message box was in use, so nothing was sent': 'content.messageBoxInUse',
      'ChatGPT would not send the message': 'content.chatgptWouldNotSend',
      'ChatGPT would not replace the New Chat draft': 'content.chatgptWouldNotReplaceDraft',
      'that answer had no text to continue from': 'content.answerNoText',
      'the answer never stopped changing, so nothing was written': 'content.answerNeverSettled',
      'the app still sees this chat working': 'content.chatStillWorking'
    };
    if (exact[text]) return uiLanguage === 'zh-CN' ? t(exact[text]) : text;
    // Keep canonical/internal producer codes unchanged in English. They are translated only
    // for the Simplified-Chinese presentation layer; control flow and diagnostics keep raw values.
    if (uiLanguage !== 'zh-CN') return text;
    if (text === 'no_api_key') return t('content.goalNoApiKey');
    if (text === 'no_conversation') return t('content.goalNoConversation');
    if (text === 'provider_completion_error') return t('content.goalProviderCompletionError');
    if (text === 'loop_stop_refused') return t('content.goalLoopStopRefused');
    if (text === 'timeout_or_cancelled') return t('content.goalTimeoutOrCancelled');
    if (text === 'rate_limited') return t('content.goalRateLimited');
    let wrapped = /^request_failed:\s*(.+)$/.exec(text);
    if (wrapped) return t('content.goalRequestFailed', { detail: wrapped[1] });
    wrapped = /^goal_failed:\s*(.+)$/.exec(text);
    if (wrapped) return t('content.goalFailed', { detail: wrapped[1] });
    wrapped = /^timeout_or_cancelled(?::.*)?$/.exec(text);
    if (wrapped) return t('content.goalTimeoutOrCancelled');
    wrapped = /^http_(\d{3})(?::.*)?$/.exec(text);
    if (wrapped) return t('content.goalHttpFailed', { status: wrapped[1] });
    const noAnswer = /^(ChatGPT helper|Offline templates|custom endpoint|OpenRouter) did not answer$/.exec(text);
    if (noAnswer) {
      const provider = noAnswer[1] === 'ChatGPT helper' ? t('content.goalProvider.chatgpt')
        : noAnswer[1] === 'Offline templates' ? t('content.goalProvider.templates')
          : noAnswer[1] === 'custom endpoint' ? t('content.goalProvider.custom') : 'OpenRouter';
      return t('content.goalProviderNoAnswer', { provider });
    }
    return text;
  }
  ...
  /** Stable Send-state silence before one exact-document stale-render recovery request. */
  ...
  async function loadRenderPreference() {
    ...
    try {
      ...
      localLanguagePreference = validLanguagePreference(stored[UI_LANGUAGE_KEY]) ? stored[UI_LANGUAGE_KEY] : 'system';
      refreshUiLanguage();
    } catch {
  ...
  function emit(observation) {
    ...
    const takeUtf8 = (value, budget) => {
      ...
      const marker = '\n\n[Chat in DaVinci: browser observation truncated to fit transport.]';
      ...
      if (users.length === 1 && matchesSubmittedUser(users[0], pendingObjectiveSend.text)) {
        ...
        void ask({ type: 'goal_objective', conversationId: id, text: carried, mode: carriedMode }).then((reply) => {
          ...
          if (reply && reply.ok === true) {
            ...
            objectiveError = replyError(reply) || t('content.goalSaveFailed');
          }
          injectStage();
        });
      }
  ...
  const ACTIVITY_KINDS = new Set(['edit', 'create', 'delete', 'move', 'read', 'search', 'browse', 'run', 'process', 'screen', 'input', 'clipboard', 'session', 'agent', 'other']);

  function activityKindText(summary) {
    const kind = ACTIVITY_KINDS.has(summary && summary.kind) ? summary.kind : 'other';
    return t(`content.activity.kind.${kind}`);
  }

  /** Translate only the browser presentation. Stored summaries remain canonical English facts. */
  function displayTitle(entry) {
    const raw = entry && entry.summary && typeof entry.summary.title === 'string' ? entry.summary.title.trim() : '';
    if (uiLanguage !== 'zh-CN') return raw;
    const action = activityKindText(entry.summary || {});
    if (entry.outcome === 'tool_rejected') return t('content.activity.refusedAction', { action });
    if (entry.outcome && !['ok', 'process_exit_nonzero'].includes(entry.outcome)) {
      return t('content.activity.failedAction', { action });
    }
    const exact = new Map([
      ['Applied a patch', '应用补丁'],
      ['Listed approved folders', '列出已授权文件夹'],
      ['Listed open windows', '列出已打开窗口'],
      ['Looked at the screen', '查看屏幕'],
      ['Filled in a field', '填写字段'],
      ['Moved the pointer', '移动指针'],
      ['Pressed keys', '按下按键'],
      ['Focused a window', '聚焦窗口'],
      ['Read the clipboard', '读取剪贴板'],
      ['Replaced the clipboard text', '替换剪贴板文本'],
      ['Listed recent recordings', '列出最近记录'],
      ['Opened one recorded tool call', '打开一条已记录工具调用'],
      ['Continued reading a recorded session', '继续读取已记录会话'],
      ['Read a recorded session', '读取已记录会话'],
      ['Reported the finished task', '报告已完成任务'],
      ['Checked agent status', '检查 Agent 状态']
    ]);
    if (exact.has(raw)) return exact.get(raw);
    let match = /^Read (\d+) paths?$/i.exec(raw);
    if (match) return `读取 ${match[1]} 个路径`;
    match = /^(Edited|Created|Deleted|Moved) (\d+) files?$/i.exec(raw);
    if (match) {
      const verb = { edited: '修改', created: '创建', deleted: '删除', moved: '移动' }[match[1].toLowerCase()];
      return `${verb} ${match[2]} 个文件`;
    }
    match = /^Created (\d+) worker agents?$/i.exec(raw);
    if (match) return `创建 ${match[1]} 个 Worker`;
    match = /^Messaged (\d+) agents?$/i.exec(raw);
    if (match) return `向 ${match[1]} 个 Agent 发送消息`;
    if (/^Messaged the prime agent$/i.test(raw)) return '向 Prime 发送消息';
    match = /^Searched recordings(?:\s+(.+))?$/i.exec(raw);
    if (match) return match[1] ? `搜索记录 ${match[1]}` : '搜索记录';
    match = /^(.+) and (\d+) more$/i.exec(raw);
    if (match) {
      const base = displayTitle({ ...entry, summary: { ...entry.summary, title: match[1] } });
      return `${base}，另有 ${match[2]} 项`;
    }
    const prefixes = [
      ['Command failed ', '命令失败：'],
      ['Started ', '启动：'],
      ['Ran ', '运行：'],
      ['Stopped session ', '停止会话 '],
      ['Interrupted session ', '中断会话 '],
      ['Wrote to session ', '写入会话 '],
      ['Waited on session ', '等待会话 '],
      ['Read ', '读取 '],
      ['Searched ', '搜索 '],
      ['Edited ', '修改 '],
      ['Created ', '创建 '],
      ...
      ['Typed', '输入'],
      ['Waited', '等待'],
      ['Acted on the desktop', '操作桌面'],
      ['Messaged ', '发送消息给 ']
    ];
    for (const [from, to] of prefixes) {
      if (raw === from || raw.startsWith(from)) return `${to}${raw.slice(from.length)}`;
    }
    return action;
  }

  function displayDetail(summary) {
    const raw = summary && typeof summary.detail === 'string' ? summary.detail.trim() : '';
    if (!raw || uiLanguage !== 'zh-CN') return raw;
    let match = /^lines\s+(\d+)[–-](\d+)(?:\s+of\s+(\d+))?$/i.exec(raw);
    if (match) return match[3] ? `第 ${match[1]}–${match[2]} 行，共 ${match[3]} 行` : `第 ${match[1]}–${match[2]} 行`;
    match = /^from line\s+(\d+)$/i.exec(raw);
    if (match) return `从第 ${match[1]} 行开始`;
    match = /^(\d+)\s+(match|matches)(?:\s+in file contents)?$/i.exec(raw);
    if (match) return `${match[1]} 个匹配项${/file contents/i.test(raw) ? ' · 文件内容' : ''}`;
    match = /^(\d+)\s+(window|windows)$/i.exec(raw);
    if (match) return `${match[1]} 个窗口`;
    match = /^(\d+)\s+(control|controls)$/i.exec(raw);
    if (match) return `${match[1]} 个控件`;
    match = /^(\d+)\s+(action|actions)$/i.exec(raw);
    if (match) return `${match[1]} 个操作`;
    match = /^(\d+)\s+(session|sessions)$/i.exec(raw);
    if (match) return `${match[1]} 个会话`;
    match = /^(\d+)\s+(entry|entries)$/i.exec(raw);
    if (match) return `${match[1]} 条记录`;
    match = /^(\d+)\s+(message|messages)$/i.exec(raw);
    if (match) return `${match[1]} 条消息`;
    if (/^in file contents$/i.test(raw)) return '文件内容';
    if (/^by name$/i.test(raw)) return '按名称';
    return raw.replace(/\s\+(\d+)\s+more$/i, ' +$1 个');
  }

  /** Translate only exact app-owned stream prose; unknown/external evidence stays raw. */
  function streamRuntimeText(value) {
    const text = typeof value === 'string' ? value : '';
    if (!text || uiLanguage !== 'zh-CN') return text;
    const exact = {
      'ChatGPT marked the turn interrupted': 'content.runtime.turnInterrupted',
      'no visible output and no progress for ten minutes': 'content.runtime.turnStalled',
      'a new user message replaced the unfinished turn': 'content.runtime.turnReplaced',
      'No visible progress for ten minutes. The turn is still marked as generating.': 'content.runtime.noVisibleProgress',
      'ChatGPT stopped updating this answer after its Send control returned. Reloading the same chat may recover the server result.': 'content.runtime.staleRender',
      'ChatGPT finished its answer but its composer is still stuck on Stop. Recovering this page before delivering the queued message.': 'content.runtime.composerStuck'
    };
    if (exact[text]) return t(exact[text]);
    const reasons = {
      'missing connector attribution': 'content.repair.reason.missingAttribution',
      'an interrupted response': 'content.repair.reason.interruptedResponse',
      'a missing browser tab': 'content.repair.reason.missingTab',
      'an unresponsive open turn': 'content.repair.reason.unresponsiveTurn',
      'a goal reply nothing collected': 'content.repair.reason.uncollectedGoal',
      'an uncollected compaction ticket': 'content.repair.reason.uncollectedCompaction'
    };
    let match = /^Trying to reload chat to recover (.+)…$/.exec(text);
    if (match && reasons[match[1]]) return t('content.repair.tryReload', { reason: t(reasons[match[1]]) });
    match = /^(Reopened|Reloaded) chat to recover (.+)\.$/.exec(text);
    if (match && reasons[match[2]]) return t(match[1] === 'Reopened' ? 'content.repair.reopened' : 'content.repair.reloaded', { reason: t(reasons[match[2]]) });
    match = /^(Reopen|Reload) failed while recovering (.+); will retry\.$/.exec(text);
    if (match && reasons[match[2]]) return t(match[1] === 'Reopen' ? 'content.repair.reopenFailed' : 'content.repair.reloadFailed', { reason: t(reasons[match[2]]) });
    return text;
  }

  function labelText(entry) {
    const title = displayTitle(entry);
    const detail = displayDetail(entry.summary);
    return detail ? `${title} · ${detail}` : title;
  }

  function callOutcomeText(outcome) {
    if (uiLanguage !== 'zh-CN') return outcome || 'ok';
    if (!outcome || outcome === 'ok') return t('content.callOutcome.ok');
    if (outcome === 'tool_rejected') return t('content.callOutcome.rejected');
    if (outcome === 'process_exit_nonzero') return t('content.callOutcome.nonzero');
    return t('content.callOutcome.failed');
  }

  function callDurationText(durationMs) {
    return durationMs ? t('content.callDurationMs', { duration: Math.round(durationMs) }) : '';
  }
  ...
  function displayMetric(summary) {
    ...
    if (uiLanguage === 'zh-CN') {
      if (metric === 'running') return t('content.activity.metric.running');
      if (metric === 'refused') return t('content.activity.metric.refused');
      if (metric === '✕ failed') return `✕ ${t('content.activity.metric.failed')}`;
      if (metric === '✕ timed out') return `✕ ${t('content.activity.metric.timedOut')}`;
      const exit = /^✕\s+exit\s+(-?\d+)$/i.exec(metric);
      if (exit) return `✕ ${t('content.activity.metric.exit', { code: exit[1] })}`;
      const lines = /^(\d+)\s+lines?$/i.exec(metric);
      if (lines) return t('content.activity.metric.lines', { count: lines[1] });
      const counts = /^(\d+)\s+(paths?|files?|matches?|windows?|controls?|actions?|sessions?|entries|messages?)$/i.exec(metric);
      if (counts) {
        const nouns = {
          path: '个路径', paths: '个路径', file: '个文件', files: '个文件', match: '个匹配项', matches: '个匹配项',
          window: '个窗口', windows: '个窗口', control: '个控件', controls: '个控件', action: '个操作', actions: '个操作',
          session: '个会话', sessions: '个会话', entry: '条记录', entries: '条记录', message: '条消息', messages: '条消息'
        };
        return `${counts[1]} ${nouns[counts[2].toLowerCase()]}`;
      }
    }
    return metric;
  }
  ...
  /**
   * Paint one compact metric without changing its canonical text.
   *
   * File deltas carry two meanings in one string. Keeping "+N" and "−N" as one text node
   * made the Overwrite stream lose the add/remove colour distinction even though the
   * underlying summary still retained both counts. Split only that exact metric shape;
   * every other metric remains ordinary text.
   */
  function paintMetric(node, metric) {
    if (!node) return;
    ...
    const parts = [];
    if (match[1]) parts.push(document.createTextNode(match[1]));
    if (match[2]) {
      const added = document.createElement('span');
      added.className = 'clf-metric-add';
      added.textContent = match[2];
      parts.push(added);
    }
    if (match[2] && match[3]) parts.push(document.createTextNode(' '));
    if (match[3]) {
      const removed = document.createElement('span');
      removed.className = 'clf-metric-remove';
  ...
  async function confirmLiveRequestOwners(calls, ownerConversation) {
    ...
    for (const call of calls) {
      if (!call || !call.requestId || !call.turnId || byRequest.has(call.requestId)) continue;
  ...
  async function refreshFiber(settled = null) {
    ...
    if (askedConversation) {
      // Request ownership is admitted only with the recorder's exact durable local turn id.
      //
      ...
      //
      // `ownerConversation` proves the chat, while `activeLocalTurnId` / settledTurnOwner()
      // proves the exact COS turn. A Fiber page turn id is recycled presentation metadata and
      // is never sent as recorder authority. Calls without one exact local owner stay
      // uncorrelated and therefore fail closed at the product gateway.
      //
      ...
      for (const source of ['calls', 'requests']) {
        for (const turn of answer.turns) {
          ...
          const localTurnId = turn === ownedPageTurn && activeLocalTurnId
            ? activeLocalTurnId
            : settledTurnOwner(turn);
          if (!localTurnId) continue;
          for (const call of turn[source] || []) {
            if (!call || !call.requestId || ownerSeen.has(call.requestId)) continue;
            ownerSeen.add(call.requestId);
            ownerCalls.push({ ...call, turnId: localTurnId });
          }
        }
      }
  ...
  function applyLabel(block, entry, hidden) {
    ...
    const wantedTitle = displayTitle(entry);
    const wantedDetail = displayDetail(entry.summary);
    ...
    label.textContent = wantedTitle;
    label.lang = uiLanguage;
    label.classList.add('clf-tool-title');
    label.setAttribute(
      'data-clf-tip',
      `${entry.tool} — ${callOutcomeText(entry.outcome)}${callDurationText(entry.durationMs)}` +
        `\n${t('content.originalLabel', { label: block.dataset.clfOriginal })}`
    );
    ...
    if (wantedWho) {
      ...
      whoNode.setAttribute('data-clf-tip', t('content.runByAgent', { agent: wantedWho }));
    ...
    if (wantedMetric) {
      ...
      paintMetric(metricNode, wantedMetric);
    ...
    if (wantedWhen) {
      ...
      whenNode.setAttribute('data-clf-tip', new Date(entry.time).toLocaleString(localeForFormatting()));
  ...
  function clockText(time) {
    ...
    try {
      return new Date(time).toLocaleTimeString(localeForFormatting());
    } catch {
      return '';
    }
  }
  ...
  function setFolded(block, anchor, folded, text) {
    ...
    node.setAttribute(
      'data-clf-tip',
      t('content.foldedCallsTip', { count: folded.length })
    );
    if (!chip) {
  ...
  function foldedRow(call) {
    ...
    if (wantedMetric) {
      ...
      paintMetric(metric, wantedMetric);
      line.append(metric);
    }
    return line;
  }
  ...
  function applyPageLabel(block, seen) {
    ...
    const label = CLF_DOM.toolLabel(block);
    if (!label) return;
    // A connector row has stable semantic state even when ChatGPT's visible caption is English:
    // the MAIN-world descriptor gives the tool name and whether its result has arrived. Use only
    // that semantic evidence for live localisation; never scrape/translate arbitrary page text.
    let wantedLabel = seen.tool;
    if (seen.answered === false && I18N) {
      const key = `content.tracker.tool.${seen.tool}`;
      const translated = t(key);
      wantedLabel = translated === key ? t('content.tracker.usingTool') : translated;
    }
    if (block.dataset.clfPage === seen.tool && label.textContent === wantedLabel) return;
    if (!block.dataset.clfOriginal) block.dataset.clfOriginal = label.textContent || '';
    block.dataset.clfPage = seen.tool;
    label.textContent = wantedLabel;
    if (wantedLabel !== seen.tool) label.lang = uiLanguage;
    else label.removeAttribute('lang');
    label.classList.add('clf-tool-title');
    label.title =
      `${seen.path || seen.tool}${seen.app ? ` — ${seen.app}` : ''}\n` +
      `${t('content.pageRecordSource')}\n` +
      t('content.originalLabel', { label: block.dataset.clfOriginal });
  ...
  function streamToolDetails(entry) {
    ...
    const outcome = uiLanguage === 'zh-CN'
      ? entry.outcome === 'ok' ? t('content.activity.completed') : entry.outcome === 'error' ? t('content.activity.failed')
        : entry.outcome === 'rejected' ? t('content.activity.metric.refused') : t('content.activity.unknown')
      : entry.outcome === 'ok' ? 'completed' : entry.outcome === 'error' ? 'failed'
    ...
    if (changes.length > 12) lines.push({
      kind: 'more',
      text: uiLanguage === 'zh-CN'
        ? t('content.stream.moreChangedFiles', { count: changes.length - 12 })
        : `${changes.length - 12} more changed files`
    });
    return lines;
  }
  ...
  function streamRow(entry, expandedTools) {
    ...
    if (entry.kind === 'tool_call') {
      body.textContent = entry.summary && entry.summary.title ? displayTitle(entry) : t('content.ranTool', { tool: entry.tool || t('content.tool') });
      if (entry.summary && entry.summary.detail) {
        const detail = document.createElement('span');
        detail.className = 'clf-tool-detail';
        detail.textContent = displayDetail(entry.summary);
        body.append(' ', detail);
      }
    } else if (entry.kind === 'agent_message') {
      body.textContent = `${entry.from || t('content.agent')} → ${entry.to || t('content.agent')}: ${entry.text || ''}`;
    } else if (entry.kind === 'page_tool') {
      body.textContent = entry.label || t('content.chatgptTool');
    } else if (entry.kind === 'turn_start') {
      body.textContent = t('content.turnStarted');
    } else if (entry.kind === 'turn_end') {
      const outcome = ['completed', 'failed', 'stopped', 'interrupted', 'stalled', 'unknown'].includes(entry.outcome)
        ? entry.outcome : 'unknown';
      body.textContent = `${t(`content.turnEnd.${outcome}`)}${entry.detail ? ` · ${streamRuntimeText(entry.detail)}` : ''}`;
    } else {
      body.textContent = streamRuntimeText(entry.text || '');
    }
    row.append(body);
   ...
   *
   * Exact names, never a prefix: `Chat in DaVinci Backup` would be somebody else's
   * connector, and a prefix test would have this app vouch for its traffic.
   */
  const OUR_CONNECTORS = ['Chat in DaVinci'];
  ...
  async function pullActivity() {
    ...
    try {
      ...
      if (typeof data.sessionId === 'string' && data.sessionId) observed.session = data.sessionId;
      job = data.job || null;
  ...
  function controlState(input) {
    ...
    if (phase === 'delivering') {
      return {
        mode: 'busy',
        label: nativePhaseLabel(phase) || t('content.saving'),
        hint: presentationError(error, t('content.briefWaitingStore')) || t('content.briefWaitingStore'),
        action: 'cancel'
      };
    }
    ...
    if (job && job.busy && error && !phase) {
      ...
      return { mode: 'error', label: t('content.paused'), hint: presentationError(error, t('content.compactionFailed')), action: 'cancel' };
    }

    if (job && job.busy) {
      if (job.stage === 'opening') {
        return { mode: 'busy', label: t('content.opening'), hint: t('content.handoffSavedOpening'), action: 'cancel' };
      }
      if (job.stage === 'waiting-for-browser') {
        return {
          mode: 'waiting',
          label: t('content.waiting'),
          hint: presentationError(job.error, t('content.appOpeningFresh')) || t('content.appOpeningFresh'),
          action: 'cancel'
        };
      }
      ...
      return {
        mode: 'busy',
        label: nativePhaseLabel(phase) || t('content.asking'),
        hint: t('content.chatgptWritingHandoff'),
        action: 'cancel'
      };
    }
    /**
    ...
    if (job && job.stage === 'done') {
      return { mode: 'done', label: t('content.opened'), hint: t('content.freshChatOpen'), action: 'start' };
    }
    if (job && job.stage === 'failed') {
      if (job.error === 'cancelled') {
        return { mode: 'idle', label: t('content.compact'), hint: t('content.resumeCancelled'), action: 'start' };
      }
      return { mode: 'error', label: t('content.failed'), hint: presentationError(job.error, t('content.compactionFailed')) || t('content.compactionFailed'), action: 'start' };
    }
    if (pressedAt > 0 && now - pressedAt < PRESS_GRACE_MS) {
      return { mode: 'busy', label: t('content.starting'), hint: '', action: 'none' };
    }
    if (error) return { mode: 'error', label: t('content.failed'), hint: presentationError(error, t('content.compactionFailed')), action: 'start' };
    if (disconnected) {
      return {
        mode: 'off',
        label: t('content.compact'),
        hint: t('content.browserDisconnected'),
        action: 'none'
      };
    }
    if (!connected) {
    ...
    if (!conversationId) {
      ...
      return {
        mode: 'off',
        label: t('content.compact'),
        hint: t('content.nothingToCompact'),
        action: 'none'
      };
    }
    return { mode: 'idle', label: t('content.compact'), hint: '', action: 'start' };
  }
  ...
  function settingsView(input) {
    ...
    const from = threshold > 0 ? t('content.fromTokens', { tokens: roundK(threshold) }) : '';
    ...
    return {
      ...
      tip: [
        auto ? t('content.autoOn', { from: from ? (uiLanguage === 'zh-CN' ? `，${from}` : `, ${from}`) : '' }) : t('content.autoOff'),
        blocked === 'worker'
          ? t('content.goalOffPrime')
          : blocked === 'blocked'
            ? t('content.goalOffBlocked')
            : fresh
            ? !hasKey
              ? t('content.freshNoKey')
              : objective
                ? t('content.openingOnGoal')
                : t('content.addGoalOrLoop')
            : position === 'off'
              ? t('content.goalAndLoopOff')
              : position === 'loop'
                ? hasKey
                  ? t('content.loopOn')
                  : t('content.loopOnNoKey')
                : hasKey
                  ? objective
                    ? t('content.goalOnOwn')
                    : t('content.goalOn')
                  : t('content.goalOnNoKey')
      ].join('\n'),
      rows: [
        {
          key: 'autoCompact',
          label: t('content.autoCompaction'),
          note:
            blocked === 'worker'
              ? t('content.workerNoAuto')
              : blocked === 'blocked'
                ? t('content.blockedNoAuto')
                : auto
                  ? from || t('content.thresholdInApp')
                  : t('content.compactByHand'),
          on: auto,
          warn: false,
          disabled: fenced
        }
      ...
      mode: fresh
        ...
        : {
            value: position,
            options: [
              { value: 'off', label: t('content.modeOff'), hint: t('content.modeOffHint') },
              {
                value: 'goal',
                label: t('content.goal'),
                hint: t('content.goalOptionHint', { model: modelLabel(goal && goal.model) })
              },
              {
                value: 'loop',
                label: t('content.modeLoop'),
                hint: t('content.loopOptionHint', { model: modelLabel(goal && goal.model) })
              }
            ],
            ...
            note:
              blocked === 'worker'
                ? t('content.primeWritesHere')
                : blocked === 'blocked'
                  ? t('content.blockedInApp')
                  : !hasKey
                  ? t('content.openRouterKeyNeeded')
                  : position === 'loop'
                    ? t('content.repliesForever')
                    : position === 'goal'
                      ? t('content.modeGoalReplies')
                      : t('content.modeNoReplies'),
            warn: !hasKey || fenced,
            disabled: fenced
          },
      /**
      ...
      objective: {
        ...
        actions: fresh
          ? [
              {
                mode: 'goal',
                label: t('content.addGoal'),
                hint: t('content.addSpecificGoalHint')
              },
              {
                mode: 'loop',
                label: t('content.addSpecificLoop'),
                hint: t('content.addSpecificLoopHint')
              }
            ]
          : [
              {
                ...
                // as a mode switch.
                mode: driving,
                label: objective ? t('content.editTask') : t('content.addTask'),
                ...
                hint:
                  position === 'off'
                    ? t('content.pickModeFirst')
                    : objective
                      ? t('content.changeTaskMode', { mode: driving === 'loop' ? t('content.modeLoop') : t('content.goal') })
                      : t('content.writeTaskMode', { mode: driving === 'loop' ? t('content.modeLoop') : t('content.goal') })
              }
            ],
        available: hasKey && !blocked,
        unavailable:
          blocked === 'worker'
            ? t('content.workerDrivenByPrime')
            : blocked === 'blocked'
              ? t('content.blockedReleaseFirst')
              : !hasKey
              ? t('content.addKeyFirst')
              : ''
      },
      ...
      action: {
        label:
          fenced
            ? t('content.compactUnavailable')
            : compact.action === 'cancel'
              ? t('content.cancelCompaction')
              : t('content.compactNow'),
        hint:
          blocked === 'worker'
            ? t('content.workerNoCompact')
            : blocked === 'blocked'
              ? t('content.blockedNoCompact')
              : compact.hint,
  ...
  const NATIVE_PHASE_KEYS = {
    requested: 'content.starting',
    interrupting: 'content.stopping',
    settling: 'content.settling',
    prompting: 'content.asking',
    waiting: 'content.writing',
    delivering: 'content.saving'
  };
  ...
  function buildControl() {
    ...
    cancel.textContent = '×';
    cancel.setAttribute('aria-label', t('content.cancelCompact'));
    ...
    blocked.textContent = t('content.chatBlocked');
    blocked.setAttribute('data-clf-tip', t('content.chatBlockedTip'));
    blocked.hidden = true;
  ...
  function buildMenu() {
    ...
    root.setAttribute('aria-label', t('content.settings'));
    root.hidden = true;
  ...
  function nativePhaseLabel(phase) {
    const key = NATIVE_PHASE_KEYS[phase];
    return key ? t(key) : '';
  }
  ...
  async function saveObjective(text, mode) {
    ...
    try {
      ...
      if (where.state === 'moving') {
        ...
        objectiveError = t('content.chatStillOpening');
        return;
      }
      if (where.state === 'new') {
      ...
      if (!reply || reply.ok !== true) {
        objectiveError = replyError(reply) || t('content.appDidNotAnswer');
        return;
      }
  ...
  function buildMode(mode) {
    ...
    track.setAttribute('aria-label', t('content.modeLabel'));
  ...
  function buildObjective(objective) {
    ...
    if (!objective.editing) {
      ...
      for (const action of objective.actions) {
        ...
        word.textContent = objectiveBusy ? t('content.working') : action.label;
    ...
    input.rows = 3;
    input.placeholder = t('content.goalPlaceholder');
    ...
    save.textContent = objectiveBusy ? t('content.saving') : objective.mode === 'loop' ? t('content.saveAsLoop') : t('content.saveAsGoal');
    ...
    cancel.textContent = t('content.cancel');
    ...
    if (objective.savable === false) {
      buttons.setAttribute('data-clf-tip', t('content.pickModeFirst'));
    }
    ...
    if (objective.text) {
      ...
      clear.textContent = t('content.clear');
  ...
  function renderControl() {
    ...
    control.cancel.setAttribute('aria-label', t('content.cancelCompact'));
    control.blocked.textContent = t('content.chatBlocked');
    control.blocked.setAttribute('data-clf-tip', t('content.chatBlockedTip'));
    ...
    control.button.setAttribute('aria-label', t('content.settings'));
  ...
  function foldBootstrap() {
    ...
    if (bootstrap === 'worker') {
      const worker = bootstrapAgent || agent || 'a worker';
      label.textContent = uiLanguage === 'zh-CN'
        ? `${worker} · ${t('content.workerBootstrapFold')}`
        : `This is ${worker} — the instruction this app gave the worker, not something you typed`;
    } else {
      label.textContent = uiLanguage === 'zh-CN'
        ? t('content.resumeBootstrapFold')
  ...
  function compactSteps() {
    return [
      t('content.compactStep.preparing'),
      t('content.compactStep.writingHandoff'),
      t('content.compactStep.saving'),
      t('content.compactStep.opening')
    ];
  }
  ...
  function stageView(input) {
    ...
    if (job && job.busy) {
      const stage =
        job.stage === 'opening'
          ? t('content.openFreshChat')
          : job.stage === 'waiting-for-browser'
            ? t('content.waitChrome')
            : t('content.chatgptWritingHandoff');
        ...
        job.stage === 'opening' || job.stage === 'waiting-for-browser'
          ...
          : phase === 'delivering'
            ...
            : phase === 'prompting' || phase === 'waiting' || asked
              ? 1
              : 0;
      return { stage, detail: '', body: '', kind: 'compact', steps: compactSteps(), at, done: false };
    }
    ...
    if (progress?.tools?.count > 0 && now - progress.tools.since >= 3000)
      return frame(progress.tools.count === 1 ? t('content.waitLocalTool') : t('content.waitLocalTools', { count: progress.tools.count }));
    const workers = progress?.workers;
    if (workers && (workers.active > 0 || workers.failed > 0)) {
      const summary = workers.failed
        ? t('content.workerSummaryFailed', { finished: workers.finished, active: workers.active, failed: workers.failed })
        : t('content.workerSummary', { finished: workers.finished, active: workers.active });
      // Running siblings are not proof that the prime is blocked on them.
      return frame(workers.active === 1 ? t('content.workerStillRunning', { worker: workers.names?.[0] || 'Worker' }) : workers.active > 1
        ? t('content.workersStillRunning', { count: workers.active }) : t('content.workerNeedsAttention'), summary);
    }
    return input.generating ? frame(t('content.waitCurrentOperation')) : null;
  }
  ...
  function modelLabel(id) {
    ...
    if (!name) return t('content.model');
  ...
  function goalSteps() {
    return [
      t('content.goalStep.answerSettling'),
      t('content.goalStep.readingChat'),
      t('content.goalStep.writingReply'),
      t('content.goalStep.sending')
    ];
  }
  ...
  function goalStageView(goal) {
    ...
    const dest = backend === 'chatgpt'
      ? t('content.goalProvider.chatgpt')
      : backend === 'templates'
        ? t('content.goalProvider.templates')
        : goal.provider === 'custom'
          ? t('content.goalProvider.custom')
          : 'OpenRouter';
    const bar = (at, done = false) => ({ steps: goalSteps(), at, done });
    const rawFailure = goal.error || (draft && draft.stage === 'failed'
      ? draft.error || t('content.goalProviderNoAnswer', { provider: dest })
      : '');
    const failure = goalFailureText(rawFailure);
    if (failure) {
      ...
      if (goal.phase === 'retrying') {
        ...
        return { stage: t('content.retryGoalInSeconds', { seconds }), detail: failure, body: '', kind: 'goal', ...bar(at) };
      }
      return {
        stage: t('content.goalStopped'), detail: failure, body: '', kind: 'goal-error', ...bar(at),
        retryHelperInputId: rawFailure === 'goal_browser_send_unconfirmed' && typeof goal.pausedHelper === 'string'
          ? goal.pausedHelper : ''
      };
    }
    ...
    if (goal.opening) {
      if (goal.phase === 'sending') return { stage: t('content.sendChatgpt'), detail: '', body: '', kind: 'goal', ...bar(3) };
      return { stage: t('content.modelFirstMessage', { model: who }), detail: '', body: '', kind: 'goal', ...bar(2) };
    }
    if (goal.phase === 'done') {
      ...
      return { stage: t('content.goalReached'), detail: t('content.nothingSent'), body: '', kind: 'goal-done', ...bar(2, true) };
    }
    if (goal.phase === 'settling') {
      return { stage: t('content.checkAnswerFinished'), detail: '', body: '', kind: 'goal', ...bar(0) };
    }
    if (goal.phase === 'sending' && draft && draft.reply) {
      return { stage: t('content.sendChatgpt'), detail: '', body: draft.reply, kind: 'goal', ...bar(3) };
    }
    if (goal.phase === 'requesting' && !draft) {
      return { stage: t('content.sendGoalProvider', { provider: dest }), detail: who, body: '', kind: 'goal', ...bar(1) };
    }
    ...
    if (draft.stage === 'answering') {
      ...
      return { stage: t('content.modelAnswering', { model: who }), detail: '', body: draft.text || '', kind: 'goal', ...bar(2) };
    }
    if (draft.stage === 'ready') {
      // Written, not yet typed: the third segment is full and the fourth has not started.
      return { stage: t('content.modelWroteNext', { model: who }), detail: '', body: draft.reply || '', kind: 'goal', ...bar(2, true) };
    }
    return null;
  }
  ...
  function buildStage() {
    ...
    const retryHelper = document.createElement('button');
    retryHelper.className = 'clf-stage-action';
    retryHelper.type = 'button';
    retryHelper.hidden = true;
    retryHelper.addEventListener('click', async () => {
      if (!stagePanel || stagePanel.root !== root || !stagePanel.retryHelperInputId || !conversationId) return;
      const forId = conversationId;
      const inputId = stagePanel.retryHelperInputId;
      retryHelper.disabled = true;
      const reply = await ask({ type: 'goal_retry_helper', conversationId: forId, inputId }).catch(() => null);
      if (conversationId !== forId || !stagePanel || stagePanel.root !== root) return;
      if (reply?.ok === true && reply.data?.accepted === true) {
        goalError = '';
        goalDraft = null;
        setGoalPhase('requesting');
      } else {
        setGoalPhase('drafting', replyError(reply) || t('content.helperRetryChanged'));
      }
      retryHelper.disabled = false;
    });
    const close = document.createElement('button');
    close.className = 'clf-stage-close';
    close.type = 'button';
    close.textContent = '×';
    close.title = t('content.dismiss');
    close.setAttribute('aria-label', t('content.dismissGoal'));
    close.hidden = true;
    ...
    head.append(title, detail, retryHelper, close);
    ...
    return { root, title, detail, retryHelper, retryHelperInputId: '', close, steps, body, dismissKey: '' };
  }
  ...
  function injectStage() {
    ...
    stagePanel.root.lang = uiLanguage;
    ...
    stagePanel.retryHelperInputId = view.retryHelperInputId || '';
    stagePanel.retryHelper.hidden = stagePanel.retryHelperInputId === '';
    stagePanel.retryHelper.textContent = t('content.startNewHelper');
  ...
  async function cancelCompact() {
    ...
    else if (!reply || reply.ok !== true) localError = replyError(reply) || t('content.cancelCompactFailed');
    renderControl();
    void pullActivity();
  }
  ...
  function replyError(reply) {
    if (!reply) return '';
    const data = reply.data || {};
    const known = data.error === 'session_not_recorded' ? t('content.noRecordedSession')
      : data.error === 'goal_owned_elsewhere' ? t('content.goalOwnedElsewhere')
        : data.error === 'compaction_running' ? t('content.anotherCompacting')
          : data.error === 'turn_still_generating' ? t('content.waitTurnFinish') : '';
    if (uiLanguage === 'zh-CN' && known) return known;
    if (data.message) {
      const message = String(data.message).slice(0, 160);
      const localized = goalFailureText(message);
      return uiLanguage === 'zh-CN' && localized === message ? presentationError(message, t('content.failed')) : localized;
    }
    if (known) return known;
    if (data.error) return uiLanguage === 'zh-CN' ? t('content.failed') : String(data.error).slice(0, 160);
    if (reply.error === 'app_not_found') return t('content.appNotRunning');
    return reply.error ? (uiLanguage === 'zh-CN' ? t('content.failed') : String(reply.error).slice(0, 160)) : '';
  }
  ...
  */
  ...
  if (globalThis.chrome && chrome.storage && chrome.storage.onChanged) {
    const storageChanged = (changes, areaName) => {
      ...
      if (changes[UI_LANGUAGE_KEY]) {
        const value = changes[UI_LANGUAGE_KEY].newValue;
        localLanguagePreference = validLanguagePreference(value) ? value : 'system';
        if (refreshUiLanguage()) changed = true;
      }
      if (!changed) return;
      renderPreferenceReady = true;
      paint();
  ...
  /**
   * Final authority for a stale-render reload. Detection crossed two IPC hops, so re-read every
   * mutable safety fact in the exact document and exact local turn immediately before navigation.
   */
  ...
  if (globalThis.chrome && chrome.runtime && chrome.runtime.onMessage) {
    const runtimeMessage = (message, _sender, sendResponse) => {
      ...
      if (message.type === 'clf-stale-render-reload') {
        ...
        try {
          // Keep the final safety check and navigation in one JavaScript task.
