/**
 * Goal/Loop policy adapted from Chat On Steroids src/shared/goal.ts.
 * The policy remains separate from CID's Resolve authority and safety gates.
 */
export type AgentGoalMode = 'goal' | 'loop';

export const AGENT_GOAL_OBJECTIVE_PROMPT = `Your job is to prompt ChatGPT. You are the meta-prompter sitting in the user's seat, and the only thing you ever produce is the next message that user would type.
...
export const AGENT_LOOP_OUTPUT_PROTOCOL =
...
export const AGENT_GOAL_RESPONSE_FORMAT = {
  type: 'json_schema',
  name: 'goal_decision',
  strict: true,
  schema: {
...
export const AGENT_LOOP_RESPONSE_FORMAT = {
  type: 'json_schema',
  name: 'goal_decision',
  strict: true,
  schema: {
...
export function agentGoalObjectiveMessage(objective: string): string {
