...
/**
 * Sleeping workers this prime may deliberately reuse instead of opening fresh chats.
 *
 * Read-only and owner-scoped. It never reactivates a dormant family, reserves a slot or touches
 * the browser; those state changes still belong to stageMessages() after the prime explicitly
 * chooses a worker. A worker caller gets no candidates, so its later spawn attempt still reaches
 * the normal "workers must not create workers" refusal instead of learning about its siblings.
 */
export function reusableWorkersForCaller(caller: Caller): AgentInfo[] {
  requireEnabled();
  if (!caller.conversationId) throw new IdentityLostError();

  const run = runForConversation(caller.conversationId);
  const agents =
    run?.primeConversationId === caller.conversationId
      ? run.agents
      : dormantRunForPrime(caller.conversationId)?.agents ?? null;
  if (!agents) return [];
  ...
  return [...agents.values()]
    .filter(
      (agent) =>
        ...
        Boolean(agent.info.conversationId) &&
        canBeRevived(agent.info) &&
...
function makeWorker(id: string, label: string, task: string, model: string | null, reasoningEffort: ReasoningEffort | null): Agent {
  return {
    info: {
      ...
      lastSeenAt: null,
      revivable: false,
      revivalClaimCommandId: null,
      revivalClaimedAt: null,
      lastRevivalCommandId: null,
      sleptAt: null,
      contextTokens: 0
    },
    queue: []
 ...
 */
export function claimWorkerRevival(
  id: string,
  conversationId: string,
  commandId: string,
  runId?: string
  ...
  const run = scopedRun(runId);
  const agent = run?.agents.get(id);
  if (!commandId || !agent || agent.info.state !== 'waking' || agent.info.conversationId !== conversationId) return false;
  if (!agent.info.revivable) return agent.info.revivalClaimCommandId === commandId;
  agent.info.revivable = false;
  agent.info.revivalClaimCommandId = commandId;
  agent.info.revivalClaimedAt = Date.now();
  changed('critical');
  return true;
}

/** Rolls back only the browser-claim marker while the wake is otherwise still untouched. */
export function rollbackWorkerRevivalClaim(
  id: string,
  conversationId: string,
  commandId: string,
  runId?: string
  ...
  agent.info.revivalClaimCommandId = null;
  agent.info.revivalClaimedAt = null;
  changed('critical');
  return true;
}
...
export function workerRevivalClaimed(conversationId: string | null | undefined): boolean {
  ...
  return Boolean(
    ...
    agent.info.state === 'waking' &&
    !agent.info.revivable &&
    agent.info.revivalClaimCommandId &&
    !agent.info.lastRevivalCommandId
  );
}

/** Whether this exact browser command owns the current durable revival claim. */
export function workerRevivalClaimOwnedBy(
  id: string,
  conversationId: string,
  commandId: string,
  runId?: string
...
/** Durable browser-ownership cut for the current/most recent wake of this exact worker chat. */
export function workerRevivalClaimedAt(conversationId: string, runId?: string): number | null {
  const run = scopedRun(runId);
  if (!run) return null;
  const agent = [...run.agents.values()].find(
    (candidate) => candidate.info.role === 'worker' && candidate.info.conversationId === conversationId
  );
  if (!agent) return null;
  return typeof agent.info.revivalClaimedAt === 'number' && Number.isFinite(agent.info.revivalClaimedAt)
    ? agent.info.revivalClaimedAt
    : null;
}
...
export function workerRevivalDeliveredSince(
  ...
  if (workerRevivalDeliveredByCommand(id, conversationId, commandId, runId)) return true;
...
/** Exact durable delivery proof for one browser revival command. No timestamp fallback. */
export function workerRevivalDeliveredByCommand(
  id: string,
  conversationId: string,
  commandId: string,
  runId?: string
  ...
  return Boolean(
    ...
    agent.info.conversationId === conversationId &&
    commandId &&
    agent.info.lastRevivalCommandId === commandId
  );
}
...
export function noteWorkerRevived(
  ...
  if (agent.info.revivalClaimCommandId && agent.info.revivalClaimCommandId !== commandId) {
    logWarn(`multi-agent: refused a revival ack for ${id} from non-owning command ${commandId ?? '(missing)'}`);
    return false;
  }
  const now = Date.now();
  agent.info.revivable = false;
...
export function failWorkerRevival(
  ...
  if (commandId && agent.info.revivalClaimCommandId !== commandId) return null;
  agent.info.revivalClaimCommandId = null;
  agent.info.revivalClaimedAt = null;
...
/**
 * Releases a wake reservation when the local browser-command queue itself rejected the handoff.
 * This is not evidence that the worker's ChatGPT conversation failed to answer, so it must not
 * consume a wake-failure strike or publish a "could not be woken" report.
 */
export function releaseWorkerRevivalReservation(
  id: string,
  conversationId: string,
  why: string,
  runId?: string
  ...
  returnWakingWorkerToStopped(
    ...
    `The local browser-command queue rejected this revival before any payload was released: ${why}`
  );
  logWarn(`multi-agent: released ${id} revival reservation before browser ownership — ${why}`);
  changed('critical');
  return true;
}
...
export function isWorkerConversation(conversationId: string): boolean {
  ...
  return retiredWorkerForConversation(conversationId) !== null;
}
  ...
  for (const entry of entries) {
    ...
    const agent: Agent = {
      info: {
        ...
        revivalClaimCommandId:
          typeof entry.info.revivalClaimCommandId === 'string' && entry.info.revivalClaimCommandId
            ? entry.info.revivalClaimCommandId
            : null,
        revivalClaimedAt:
          typeof entry.info.revivalClaimedAt === 'number' && Number.isFinite(entry.info.revivalClaimedAt)
            ? entry.info.revivalClaimedAt
            : null,
