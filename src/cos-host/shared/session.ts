...
export interface AgentInfo {
  ...
  revivable: boolean;
  /**
   * Browser command id that currently owns this worker's durable revival claim, or null.
   *
   * Unlike lastRevivalCommandId this is pre-send authority. It is written before revival text is
   * released and cleared when that exact command reports sent/failed or the claim rolls back.
   */
  revivalClaimCommandId?: string | null;
  /**
   * Wall-clock moment the current browser revival claim became broker-owned, or null.
   * Browser turn_start evidence for this wake must be newer than this cut before it may finish
   * the worker; delayed complete batches from an earlier turn are therefore history only.
   */
  revivalClaimedAt?: number | null;
  /**
